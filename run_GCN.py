...
if mode == "validation":
    ...
    test_pred = np.array([pred[i]
                         for i in range(len(pred)) if mask[i] == True])
    test_label = np.array([labels[i]
                          for i in range(len(labels)) if mask[i] == True])
...
with open("prediction.csv", 'w') as f_out:
    f_out.write("contig_names, prediction\n")
    for key in test_to_id.keys():
        if labels[test_to_id[key]] == -1:
            f_out.write(f"{key},{pred_to_label[pred[test_to_id[key]]]}\n")
        elif labels[test_to_id[key]] == -2:
            f_out.write(f"{key},{pred_to_label[pred[test_to_id[key]]]}_like\n")
        else:
            f_out.write(f"{key},{pred_to_label[labels[test_to_id[key]]]}\n")
